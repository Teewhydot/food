...
class Env {
  // Google Maps API Key
  static String? get mapsKey => kIsWeb 
      ? const String.fromEnvironment('GOOGLE_MAPS_API_KEY')
      : dotenv.env['GOOGLE_MAPS_API_KEY'];
  
  // OpenWeatherMap API Key  
  static String? get openWeatherMapKey => kIsWeb
      ? const String.fromEnvironment('OPENWEATHERMAP_API_KEY')
      : dotenv.env['OPENWEATHERMAP_API_KEY'];
  
  // Debug Mode
  static bool get debugMode => kIsWeb
      ? const String.fromEnvironment('DEBUG_MODE').toLowerCase() == 'true' || kDebugMode
      : dotenv.env['DEBUG_MODE']?.toLowerCase() == 'true' || kDebugMode;
  
  // API Configuration
  static String get baseUrl => kIsWeb
      ? const String.fromEnvironment('BASE_URL', defaultValue: 'https://api.example.com')
      : dotenv.env['BASE_URL'] ?? 'https://api.example.com';
  
  static String get apiVersion => kIsWeb
  ...
  static bool validateRequiredEnvVars() {
    if (kIsWeb) {
      ...
      if (!mapsKeyValid && kDebugMode) {
        debugPrint('ERROR: Required environment variable GOOGLE_MAPS_API_KEY is missing');
      }
      return mapsKeyValid;
    }
